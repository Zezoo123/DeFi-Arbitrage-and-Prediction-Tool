...
describe("Arbitrage Testing", function() {
    beforeEach(async function() {
        ...
        router1 = await RouterFactory.connect(owner).deploy(await mockWETH.getAddress(), await mockDAI.getAddress(), 2);
        await router1.waitForDeployment();

        // Deploy Router 2 with exchange rate of 1
        router2 = await RouterFactory.connect(owner).deploy(await mockWETH.getAddress(), await mockDAI.getAddress(), 1);
        await router2.waitForDeployment();

        // Ensure addresses are not null
        expect(await mockWETH.getAddress()).to.not.be.null;
        expect(await mockDAI.getAddress()).to.not.be.null;
    ...
    it("should perform arbitrage between 2 routers", async function() {
        ...
        await mockWETH.connect(owner).transfer(await arbitrageur.getAddress(), ethers.parseEther("100"));

        // Approve routers to spend arbitrageur's WETH
        const router1Address = await router1.getAddress();
        const router2Address = await router2.getAddress();

        await mockWETH.connect(arbitrageur).approve(router1Address, ethers.parseEther("100"));
        await mockWETH.connect(arbitrageur).approve(router2Address, ethers.parseEther("100"));
        ...
        const daiBalanceAfterSwap1 = await mockDAI.balanceOf(await arbitrageur.getAddress());
        expect(daiBalanceAfterSwap1).to.be.greaterThan(ethers.parseEther("0"));

        // Approve router2 to spend arbitrageur's DAI
        await mockDAI.connect(arbitrageur).approve(router2Address, daiBalanceAfterSwap1);
        ...
        const wethBalanceAfterSwap2 = (await mockWETH.balanceOf(await arbitrageur.getAddress()));
